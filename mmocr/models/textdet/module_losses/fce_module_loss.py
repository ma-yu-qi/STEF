...
class FCEModuleLoss(TextSnakeModuleLoss):
    ...
    def _normalize_polygon(self, polygon: ArrayLike) -> np.ndarray:
        ...
        # 处理空数组的情况，例如通过跳过当前操作，或设置一个默认值
            temp_polygon = polygon  # 或根据需要选择其他适当的处理方式

        if temp_polygon.size > 0 and temp_polygon.shape[1] > 0:
            x = np.abs(temp_polygon[:, 0])
        else:
            # 处理空数组的情况
            # 例如，你可以设置x为None，或者根据你的需求设置一个默认值
            x = None  # 或者其他适当的默认处理

        #x = np.abs(temp_polygon[:, 0])
            
        if temp_polygon.size > 0 and temp_polygon.shape[1] > 1:
            y = np.abs(temp_polygon[:, 1])
        else:
        # 处理不满足条件的情况
            y = None  # 或其他适当的默认处理
        ...
        if y is not None and index_x is not None and len(index_x) >= 8:
        # 确保 index_x 中的所有索引都在 y 的索引范围内
            if all(idx < len(y) for idx in index_x[:8]):
                index_y = np.argmin(y[index_x[:8]])
            else:
                print("错误：index_x 中有的索引超出了 y 的范围。")
        else:
            print("错误：y 是 None 或 index_x 是 None 或 index_x 的长度小于 8。")
