...
class TextSnakeModuleLoss(SegBasedModuleLoss):
    ...
    def _generate_center_mask_attrib_maps(
        ...
        # Check if center_line has enough points before accessing
            if len(center_line) > 1 and self.vector_slope(center_line[-1] - center_line[0]) > 0.9:
                if (center_line[-1] - center_line[0])[1] < 0:
                    center_line = center_line[::-1]
                    resampled_top_line = resampled_top_line[::-1]
                    resampled_bot_line = resampled_bot_line[::-1]
            elif len(center_line) > 1:
                if (center_line[-1] - center_line[0])[0] < 0:
                    center_line = center_line[::-1]
                    resampled_top_line = resampled_top_line[::-1]
                    resampled_bot_line = resampled_bot_line[::-1]

            # 确保resampled_top_line和resampled_bot_line非空
            if len(resampled_top_line) == 0 or len(resampled_bot_line) == 0:
                continue  # 跳过当前循环迭代
    ...
    def _reorder_poly_edge(self, points: ndarray
        ...
        # Return empty ndarrays if conditions are not met
    ...
    def _resample_sidelines(self, sideline1: ndarray, sideline2: ndarray,
        ...
        if not (sideline1.ndim == sideline2.ndim == 2 and 
                sideline1.shape[1] == sideline2.shape[1] == 2 and
                sideline1.shape[0] >= 2 and 
                sideline2.shape[0] >= 2 and 
                isinstance(resample_step, float)):
            empty_array = np.array([], dtype=float).reshape(0, 2)  # Ensure the shape matches expected (N,2) but with N=0
