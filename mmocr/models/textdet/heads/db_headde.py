...
class DeformConv2dWithOffset(nn.Module):
    def __init__(self, inp, oup, kernel_size=3, stride=1, padding=1, bias=False, deformable_groups=1):
        super(DeformConv2dWithOffset, self).__init__()
        self.offset_channels = kernel_size * kernel_size * 2 * deformable_groups
        self.conv2_offset = nn.Conv2d(inp, self.offset_channels, kernel_size=kernel_size, stride=stride, padding=padding, bias=False)
        self.conv2d = DeformConv2d(inp, oup, kernel_size=kernel_size, stride=stride, padding=padding, bias=bias, groups=deformable_groups)

    def forward(self, x):
        offset = self.conv2_offset(x)
        out = self.conv2d(x, offset)
        return out
    
@MODELS.register_module()
class DBHeadde(BaseTextDetHead):
